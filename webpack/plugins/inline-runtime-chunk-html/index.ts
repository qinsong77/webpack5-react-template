...
const PLUGIN = 'InlineRuntimeChunkPlugin'
...
export default class InlineRuntimeChunkPlugin {
    ...
    compiler.hooks.compilation.tap(PLUGIN, (compilation: Compilation): void => {
      ...
      if (runtimeChunkOption) {
        ...
        hooks.alterAssetTags.tapAsync(PLUGIN, (data, cb): unknown => {
          ...
          data.assetTags.scripts.forEach(
            this._inlineRuntimes(runtimeUris, compilation)
          )
          return cb(null, data)
        })
      }
    ...
    compilation.entrypoints.forEach((entry) => {
      ...
      if (runtimeChunk) {
        runtimeChunk.files.forEach((file) =>
          result.push({ file, url: publicPath + file })
        )
      }
    })
  ...
  private _getAssetSourceCode(
    compilation: Compilation,
    file: string
  ): string | undefined {
    const entries = Object.entries(compilation.assets)
    const entry: [string, sources.Source] | undefined = entries.find(
      (item) => item[0] === file
    )
